...
class CVMinimizer:
        ...
        if prev_result:
            if this_result.fun < prev_result.fun:
                if this_result.success:
                    print('This minimization improved on the previous result and completed the minimization in the iterations given. ')
                else:
                    print('This minimization improved on the previous result, but did not finish in the given number of iterations.')
            else:
                if this_result.success:
                    print('This minimization minimized to a worse solution.')
                else:
                    print('This minimization did not improve on the previous result, and did not finish in the given number of iterations.')
            if print_steps:
            ...
            if this_result.success:
                ...
                print('This minimization did not finish in the given number of iterations.')
            if print_steps:
                    print(f'This minimization results:\n\n {this_result}')

        print('--------------------------------------------------\n')
    
    def _minimize(self, guess, tol, maxiter, print_funcevals, print_steps, prev_result=None):
        if print_funcevals:
        ...
        self._print_result(this_result=simplex_res, print_steps=print_steps, prev_result=prev_result)
        return simplex_res

    def mincv_close(self, tol=0.0002, maxiter=500, print_steps=True, print_funcevals=False):
    ...
    def mincv(self, tol=0.0002, maxiter=100, print_steps=True, print_funcevals=False, guess_bounds=[[2,10],[2,10]], guesses='all'):
        ...
        self._print_result(final_step, print_steps=True, prev_result=first_step[min_idx])
